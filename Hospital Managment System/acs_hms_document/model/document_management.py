...
class DocumentDirectory(models.Model):
    ...
    def action_view_attachments(self):
        attachments = self.env['ir.attachment'].search([
            ('res_model', 'in', [self.res_model.model])])
        imd = self.env['ir.model.data']
        action = imd.xmlid_to_object('base.action_attachment')
        list_view_id = imd.xmlid_to_res_id('base.view_attachment_tree')
        form_view_id = imd.xmlid_to_res_id('base.view_document_document_form')
        kanban_view_id = imd.xmlid_to_res_id('mail.view_document_file_kanban')
        result = {
            'name': action.name,
            'help': action.help,
            'type': action.type,
            'views': [[kanban_view_id, 'kanban'], [list_view_id, 'tree'], [form_view_id, 'form']],
            ...
            'domain': "[('id','in',%s)]" % attachments.ids
        }
        return result
