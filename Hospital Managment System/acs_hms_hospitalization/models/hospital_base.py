...
class Bed(models.Model):
    ...
    def _get_patient(self):
        for rec in self:
            patient_id = False
            ac_hists = rec.accomodation_history_ids.filtered(lambda r: r.start_date and not r.end_date )
            if ac_hists:
    ...
    state = fields.Selection([
        ('free', 'Free'),
        ('reserved', 'Reserved'),
        ('occupied', 'Occupied')], string='Status', default="free")
    ward_id = fields.Many2one('hospital.ward', ondelete='restrict', string='Ward/Room')
    accomodation_history_ids = fields.One2many("patient.accomodation.history","bed_id",
...
class ACSHospitalWard(models.Model):
    ...
    institution = fields.Many2one('res.partner', ondelete="restrict", 
        string='Institution', help='Medical Center')
...
class ACSHospitalBuilding(models.Model):
    ...
    institution = fields.Many2one('res.partner', string='Institution',
     domain=[('is_company','=',True)], ondelete="restrict", help='Medical Center')
