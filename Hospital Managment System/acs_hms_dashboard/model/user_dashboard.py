...
class Users(models.Model):
    ...
    def _compute_dashboard_data(self):
        ...
        open_invoice_domain = self.get_filter('date_invoice')
        ...
        self.birthday_patients = Patient.search_count([('dob', 'like', today_month_day)])
        self.birthday_employees = self.env['hr.employee'].search_count([('birthday', 'like', today_month_day)])
    ...
    def open_invoices(self):
        ...
        action['domain'] = self.get_filter('date_invoice') + [('type','=','out_invoice'),('state', 'in', ['open'])]
        return action
    ...
    def open_birthday_patients(self):
        ...
        today = datetime.now()
        today_month_day = '%-' + today.strftime('%m') + '-' + today.strftime('%d')
        patient_ids = Patient.search([('dob', 'like', today_month_day)])
        action = self.env.ref('acs_hms.action_patient').read()[0]
